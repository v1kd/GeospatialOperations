...
class hull implements FlatMapFunction<Iterator<Coordinate>, Coordinate>,
		Serializable {
	...
	public Iterable<Coordinate> call(Iterator<Coordinate> crd) {
		...
		try {
			while (crd.hasNext()) {
				...
				if (temp != null) {
					Coordinates.add(new WKTReader().read("Point("
							+ String.valueOf(temp.x) + " "
							+ String.valueOf(temp.y) + ")"));
...
public class convexHull {
	public static Function<String, Coordinate> parseData = new Function<String, Coordinate>() {
		...
		public Coordinate call(String s) throws Exception {
			...
			if (parts.length == 2) {
				coordinate = new Coordinate(Double.parseDouble(parts[0]),
	...
	public static void main(String[] args) throws FileNotFoundException {
		if (args.length <= 0) {
			System.out
					.println("We require input file path, output file path and number of partitions argument to proceed further.");
			System.out
					.println("Usage: java convexHull <input file path> <output file path> <noOfPartitions>");
			System.exit(0);
		}
		...
		SparkConf conf = new SparkConf().setAppName("Group6-convexHull");
		...
		// // Repartition to 1 partition in order to apply 'convex hull' on all
		// the Coordinate objects obtained from individual partitions
		JavaRDD<Coordinate> calculatedHull = localHull.coalesce(1).distinct()
				.cache();
		//
		// // Perform Convex hull operation
		JavaRDD<Coordinate> globalHull = calculatedHull.mapPartitions(
				new hull()).distinct();
		//

		// Map to a tuple to sort the Points
		JavaPairRDD<Coordinate, Boolean> coordinateTupleRDD = globalHull
				.mapToPair(new CoordinatePairFunction());

		// Sort the points
		JavaPairRDD<Coordinate, Boolean> sortedCoordinateTupleRDD = coordinateTupleRDD
				.sortByKey(new CoordinateComparator());

		// Map to points RDD
		JavaRDD<Coordinate> finalSortedCoordinateRDD = sortedCoordinateTupleRDD
				.map(new TupleToCoordinateMapFunction());

		JavaRDD<String> outputData = finalSortedCoordinateRDD
				.map(parseOutputData);
		// Write to a file
